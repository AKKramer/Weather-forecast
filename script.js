...
// Creates a variable to store position info
  var startPos;
  // A predefined functions that asks for the user's location data. 
  var geoSuccess = function (position) {
    // assigns the users position data to the external var startPos
    startPos = position;
    // Assigns the latitude & longitude to respective variables
    lat = startPos.coords.latitude;
    lon = startPos.coords.longitude;
    // calls the findCity function and passes in the user coordinates. 
    findCity(lat, lon)
  };
...
// same action performed for getting the intial weather lines 87-95. 
  ...
  // if the current history if greater than 0
  if (history.length > 0) {
    // check to see if the search is already saved. 
    searchWeather(history[history.length - 1]);
  }
  // if the history length is already 0, then create a new row and add the data. 
